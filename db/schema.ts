...
export const brokerAccounts = sqliteTable('broker_accounts', {
  ...
  uniqueIndex('idx_broker_accounts_user_broker_provider').on(table.userId, table.broker, table.providerAccountId),
...
export const securityProviderMappings = sqliteTable('security_provider_mapping', {
  securityId: text('security_id').notNull().references(() => securities.securityId, { onDelete: 'cascade' }),
  provider: text('provider').notNull(),
  providerInstrumentId: text('provider_instrument_id').notNull(),
  providerSymbol: text('provider_symbol'),
  exchange: text('exchange'),
}, (table) => [
  primaryKey({ columns: [table.securityId, table.provider, table.providerInstrumentId] }),
  uniqueIndex('idx_security_mapping_provider_instrument').on(table.provider, table.providerInstrumentId),
]);

export const brokerHoldings = sqliteTable('broker_holdings', {
  id: text('id').primaryKey(),
  brokerAccountId: text('broker_account_id').notNull().references(() => brokerAccounts.id, { onDelete: 'cascade' }),
  securityId: text('security_id').notNull().references(() => securities.securityId),
  isin: text('isin'),
  ...
  retrievedAt: text('retrieved_at').notNull().default(sql`(datetime('now'))`),
}, (table) => [
  uniqueIndex('idx_broker_holdings_account_security_exchange_product').on(table.brokerAccountId, table.securityId, table.exchange, table.productType),
...
export const portfolioPositions = sqliteTable('portfolio_positions', {
  ...
  currentValue: real('current_value'),
  unrealizedPnl: real('unrealized_pnl'),
  lastCalculatedAt: text('last_calculated_at').notNull().default(sql`(datetime('now'))`),
}, (table) => [
...
export const ingestionJobs = sqliteTable('ingestion_jobs', {
  ...
  brokerAccountId: text('broker_account_id').references(() => brokerAccounts.id, { onDelete: 'cascade' }),
  ...
  index('idx_ingestion_jobs_status_priority').on(table.status, table.priority, table.createdAt),
...
export const oauthStates = sqliteTable('oauth_states', {
  ...
  createdAt: text('created_at').notNull().default(sql`(datetime('now'))`),
}, (table) => [index('idx_oauth_states_user_provider').on(table.userId, table.provider)]);
